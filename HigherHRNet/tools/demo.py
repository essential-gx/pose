...
# print(torch.multiprocessing.get_all_sharing_strategies())# 仅支持
...
def main():
    ...
    # eval() 函数用来执行一个字符串表达式，并返回表达式的值。模型在预测阶段不需要初始化参数，所有is_train为False
    # models.pose_higher_hrnet.get_pose_net(cfg, is_train=False)
    ...
    # cap = cv2.VideoCapture(vid_file)
    cap = cv2.VideoCapture('E:\\video\\1.flv')
    ...
    while True:
        ...
        with torch.no_grad():
            ...
            for idx, s in enumerate(sorted(cfg.TEST.SCALE_FACTOR, reverse=True)):
                ...
                image_resized = image_resized.unsqueeze(dim=0).cuda()
