...
def build_transforms(cfg, is_train=True):
    ...
    transforms = T.Compose(
        [
            ...
            T.RandomHorizontalFlip(coco_flip_index, output_size, flip),# 随机水平翻转
            T.ToTensor(),# 转化为张量
            T.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])# 在channel三条通道进行标准化
        ]
    )
